...
path = get_file(
    '2600-0.txt',
    origin='https://www.gutenberg.org/files/2600/2600-0.txt')

#You can open a file locally if you remove the below hash
#text = open("alice.txt").read().lower()

#If local file also hash out the next two lines
...
# build the model: a single CuDNNLSTM
print('Build model...')
model = Sequential()
...
# Modify the Batchsize and epochs (Batch size should be set at 128, increase depending upon your GPU) 
# I get 48s per Epoch on a batch size of 256 on a GTX-1060
model.fit(x, y,
          batch_size=256,
          epochs=60,
